  ...
  return (
    <Container sx={{ py: 4 }}>
      <Grid container spacing={4}>
        {loading ? (
          // لوڈنگ کے دوران سکیلیٹن شو کرے
          Array.from(new Array(3)).map((_, index) => (
            <Grid item key={index} xs={12} sm={6} md={4}>
              <Card sx={{ bgcolor: 'background.paper', opacity: 0.95 }}>
                <Box sx={{ height: 200, backgroundColor: '#f5f5f5' }} />
                <CardContent>
                  <Box sx={{ height: 24, backgroundColor: '#e0e0e0', mb: 2, borderRadius: 1 }} />
                  <Box sx={{ height: 60, backgroundColor: '#e0e0e0', borderRadius: 1 }} />
                  <Box sx={{ height: 36, backgroundColor: '#e0e0e0', mt: 2, borderRadius: 1, width: '40%' }} />
                </CardContent>
              </Card>
            </Grid>
          ))
